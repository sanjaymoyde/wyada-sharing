...
export const EarthCarousel: React.FC<EarthCarouselProps> = ({ isNight, addToCart, onView, product, progress }) => {
                            ...
                            >
                                ...
                                <div className="w-full md:w-1/2 shrink-0 flex items-center justify-center min-h-0 order-1 md:order-2">
                                    <div className="relative h-[40vh] md:h-[70vh] w-auto aspect-square flex items-center justify-center p-4">
                                        ...
                                        {!isProductImageReady && (
                                            <div className={`absolute inset-0 m-auto w-[42%] h-[42%] rounded-3xl border flex items-center justify-center ${isNight ? 'border-brand-mesa/40 bg-brand-mesa/10' : 'border-white/40 bg-white/20'} animate-pulse`}>
                                                <span className={`text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] ${textColor}`}>mesa</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div
                                    className="w-full md:w-1/2 shrink-0 flex flex-col items-start z-10 gap-1 order-2 md:order-1 mt-2 md:mt-0 cursor-default"
                                ...
                                >
                                    ...
                                    <p className={`text-sm md:text-xl font-medium leading-relaxed max-w-lg italic ${textColor} line-clamp-2 md:line-clamp-none`}>
                                        {product?.body_html && !product.body_html.includes("test") ? product.body_html.replace(/<[^>]*>/g, '') : "Ground your skin. Cleanse. Calm. Restore."}
                                    </p>
                                </div>
                            </Link>
                    ...
                    <div className="absolute bottom-32 md:bottom-36 left-0 w-full px-6 max-[390px]:px-3 md:px-12 z-40 pointer-events-none">
                        <div className="flex justify-between items-center w-full h-full relative max-[430px]:gap-1 max-[390px]:gap-2">
                            ...
                            <div
                                ...
                                onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
                            >
                                ₹{product?.variants[0]?.price ? Number(product.variants[0].price) : 799}
                            </div>
