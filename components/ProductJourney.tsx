...
const IntroPanel: React.FC<{ title: string; subtitle: string; color: string; visual: React.ReactNode }> = ({ title, subtitle, color, visual }) => (
  <div className="h-[var(--app-vh)] w-screen flex flex-col items-center justify-center flex-shrink-0 relative overflow-hidden p-6">
...
const PhilosophyPanel: React.FC<{ text: string; subtext: string }> = ({ text, subtext }) => (
  <div className="h-[var(--app-vh)] w-screen flex flex-col items-center justify-center flex-shrink-0 p-12 text-center">
...
const IngredientsPanel: React.FC<{ ingredients: typeof INGREDIENTS }> = ({ ingredients }) => (
  <div className="h-[var(--app-vh)] w-screen flex flex-col items-center justify-center flex-shrink-0 p-6 md:p-12">
...
const CTAPanel: React.FC<{ name: string; price: string }> = ({ name, price }) => (
  <div className="h-[var(--app-vh)] w-screen flex flex-col items-center justify-center flex-shrink-0">
...
const ElementSection: React.FC<{
  ...
  return (
    <div className={`h-[var(--app-vh)] w-full overflow-hidden ${bgColor} ${className}`}>
      <motion.div
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex gap-3 z-20">
        {Array.from({ length: totalPanels }).map((_, i) => (
          <button
            key={i}
            onClick={() => setIndex(i)}
            className={`w-2 h-2 rounded-full transition-all duration-300 ${index === i ? 'bg-white w-6' : 'bg-white/40 hover:bg-white/60'
              }`}
  ...
  return (
    <div id="product-journey" className="relative w-full">
      {/* 
        ...
        */}
      <div className="sticky top-0 h-[var(--app-vh)] z-40">
      ...
      {/* 
        ...
        */}
      <div className="sticky top-0 h-[var(--app-vh)] z-50 shadow-[0_-50px_50px_rgba(0,0,0,0.2)]">
